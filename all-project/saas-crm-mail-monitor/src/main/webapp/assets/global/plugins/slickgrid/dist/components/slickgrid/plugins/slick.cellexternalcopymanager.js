(function ($) {
  ...
  function CellExternalCopyManager(options) {
    ...
    */
    ...
    function getDataItemValueForColumn(item, columnDef) {
      ...
      for (var i=0; i<_ignoreFormatting.length; i++) {
        if (_ignoreFormatting[i] === columnDef.field) {
          return ValueRender(columnDef.field,item);
        }
      }
      ...
      if (columnDef.formatter) {
          //return columnDef.formatter(0, 0, ValueRender(columnDef.field,item), columnDef, item);
    	  //TODO JACKY
    	  return columnDef.formatter(0, 0, ValueRender(columnDef.field,item), columnDef, item).replace(/<[^>]+>/g,"");
      }

      // if a custom getter is not defined, we call serializeValue of the editor to serialize
      if (columnDef.editor && columnDef.editor.name!="TextSuggestionEditor"){
        var editorArgs = {
    	...
    	retVal = ValueRender(columnDef.field,item);
      }
     ...
     * get data of cell
     */
    function ValueRender(_field,item){
  	  var _value = "";
	  var tmp = item;
	  _field.replace(/[^.]+(?=\.)/g, function($){
		 tmp = $ in tmp? tmp[$] : tmp[$] = {};
	  });
	  _field.replace(/[^.]+$/g, function($){
		 _value = tmp[$];
