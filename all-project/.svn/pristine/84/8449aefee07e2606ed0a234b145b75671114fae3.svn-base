...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function buildExecl(opt){
    	if(getData().getItems().length == 0)return;
    	var simpleExeclTemplateJson = {titleName:opt.title || ""};
    	var simpleExeclColumnVOs = [];
    	for(var i = 0,j;j = getColumns()[i++];){
    		if(j.id == "_checkbox_selector")continue;
    		simpleExeclColumnVOs.push({name:j.name.replace(/<[^>]+>/g,"")});
    	}
    	var simpleExeclRowVOs = [];
    	for(var i = 0,j;j = getData().getItems()[i++];){
    		var items = [];
    		for(var a = 0,b;b = getColumns()[a++];){
    			if(b.id == "_checkbox_selector")continue;
    			//var _value = j[b.id];
    			var _value = j[b.field];
    			if(_value === undefined)_value = "";
    			if(b.formatter)_value = b.formatter(i - 1,a - 1,_value,b,j);
    			_value = _value + "";
    			items.push(_value.replace(/<[^>]+>/g,""));
    		}
    		simpleExeclRowVOs.push({values:items});
    	}
    	simpleExeclTemplateJson.simpleExeclColumnVOs = simpleExeclColumnVOs;
    	simpleExeclTemplateJson.simpleExeclRowVOs = simpleExeclRowVOs;
    	
    	if(null == document.getElementById("execlExeclForm")){
    		var execlExeclForm = document.createElement("form");   
    		execlExeclForm.setAttribute("id","execlExeclForm");
    		execlExeclForm.method = 'post';
    		execlExeclForm.action = window.ctx + "/jf/ajaxBuildExecl";
			document.body.appendChild(execlExeclForm);
			var newElement = document.createElement("input");
			newElement.setAttribute("name","simpleExeclTemplateVOJson");
			newElement.setAttribute("id","simpleExeclTemplateVOJson");
		    newElement.setAttribute("type","hidden");
		    execlExeclForm.appendChild(newElement);
    	}
    	document.getElementById("simpleExeclTemplateVOJson").setAttribute("value",JSON.stringify(simpleExeclTemplateJson));
    	document.getElementById("execlExeclForm").submit();

    }
    
    function buildExeclAll(opt){
    	try{
    		 App.blockUI({
    	          boxed: true
    	      });
    	...
    	var _opts = _pager.getOpt();
    	var pagesize = {pagesize:_opts.pagesize};
    	_pager.reload({pagesize:10000},function(){
    		try{
    		buildExecl(opt);
    		App.unblockUI();
    		_pager.reload(pagesize);
    		}catch(e){
    			console.info(e);
    		}
...
/*    	setTimeout(function(){
    ...
    $.extend(this, {
      ...
      //TODO jacky 生成execl
      "buildExecl":buildExecl,
      "buildExeclAll":buildExeclAll,
    });
